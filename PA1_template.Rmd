...
``` {r histogram_stepsperday, message=FALSE}
library(ggplot2)
...
Given the large number of days with 0 steps taken, it is not surprising that the mean steps per day (`r daymean`) is considerably lower than the median steps per day (`r daymed`).
...
Over the 61 days, the time interval that had the highest average number of steps was from 8:35 to 8:40 am, with an average of `r sprintf(maxint$maxint, fmt="%#.2f")` steps. This finding may be due to weekday activity, such as walking to work, and will need to be explored further.
...
The number of missing values is `r num_miss`, which represents `r perc_miss`% of the data. This percentage is large enough to consider imputation of the missing values. To accomplish this, the average number of steps based on the day of the week and the time interval will be calculated and imputed for missing values.
...
``` {r combined dataframe and histogram}
...
The result of imputing data for missing values is clearly seen in the comparison of mean and median steps taken per day.  Because so many days in the raw data were zero steps due to missing data, both the mean (from `r daymean` to `r daymeanImp`) and median (from `r daymed` to `r daymedImp`) steps per day increased considerably.
